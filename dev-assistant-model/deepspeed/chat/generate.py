...
def main():
    """
    Generates text samples using a pre-trained dialogue model and a set of prompts.
    The generated samples are saved to a file named `samples-{model_name}.txt` in the `data` directory.

    Args:
        --model_id (str): Name of the pre-trained model to use for generation.
        --revision (str): The revision of the model repo to use. Defaults to `None`.
        --system_prompt (str): Overrides the dialogue template's system prompt. Defaults to `None`.
    """
