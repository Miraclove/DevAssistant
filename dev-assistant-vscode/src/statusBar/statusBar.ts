...
import {getAutocompleteStatus} from "../autocompleteInstaller"

const SPINNER = "$(sync~spin)";
const DISABLE = "$(sync-ignored)";
const ENABLE = "$(sync)";
...
export function setDefaultStatus(): void {
  ...
  if(getAutocompleteStatus()){
    statusBarData.icon = ENABLE;
  }else{
    statusBarData.icon = DISABLE;
  }
